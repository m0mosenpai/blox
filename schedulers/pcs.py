from blox.cluster_state import ClusterState
...
class Pcs(SchedulingPolicy):
    ...
    def schedule(
        ...
        sorted_jobs = sorted(
            job_dict.items(),
            key=lambda x: x[1]["demand_fn"](job_dict[job]["job_gpu_demand"])
        )
        ...
        if sorted_jobs:
            ...
            print(f"number of jobs: {len(sorted_jobs)}")
            for i in range(len(sorted_jobs)):
                # include job in the queue
                n += 1

                # calculate running demand sum, mean and variance
                gpu_demand = sorted_jobs[i][1]["job_gpu_demand"]
                demand = sorted_jobs[i][1]["demand_fn"](gpu_demand)
                demand_sum += demand
                ...
                print(f"demand: {demand}, demand_mean: {demand_mean}, demand_variance: {demand_variance}")

                # calculate C^2
                c_squared = demand_variance / (demand_mean * demand_mean)
                c_squared_history.append(c_squared)
                print(f"c_squared for job {i}: {c_squared}")
